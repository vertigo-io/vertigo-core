...
public final class ConfigXmlManagerServletTest {
	...
	private static ClassLoader getUrlClassLoader() {
		return new URLClassLoader(new URL[0], ConfigXmlManagerServletTest.class.getClassLoader());
	}

	private static void startServer() throws IOException, Exception {
		server = new Server(MyNodeConfig.WS_PORT);
		final WebAppContext context = new WebAppContext(ConfigXmlManagerServletTest.class.getClassLoader().getResource("io/vertigo/vega/webAppXml/").getFile(), "/");
		...
		context.setClassLoader(new WebAppClassLoader(ConfigXmlManagerServletTest.class.getClassLoader(), context));

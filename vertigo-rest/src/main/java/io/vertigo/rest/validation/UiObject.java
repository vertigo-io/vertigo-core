...
/**
 * Objet d'IHM, fournit les valeurs formattés des champs de l'objet métier sous-jacent.
 * Implements Map<String, Object> car la request poste des String[] que l'on reconverti en String (on prend le premier).
 * 
...
public final class UiObject<D extends DtObject> implements Map<String, Serializable>, Serializable {
	...
	public UiObject(final Class<D> dtoClass) {
		...
		this.dto = (D) DtObjectUtil.createDtObject(dtDefinitionRef.get());
		for (final DtField dtField : getDtDefinition().getFields()) {
			keysIndex.put(StringUtil.constToCamelCase(dtField.getName(), false), dtField.getName());
		}
	}
	...
	public String put(final String fieldName, final Serializable value) {
		...
		Assertion.checkArgNotEmpty(constFieldName);
		Assertion.checkNotNull(value, "La valeur formattée ne doit pas être null mais vide ({0})", fieldName);
	...
	* @throws IllegalAccessError Si le champs possède une erreur de formatage
	...
	/**
	 * Récupération des données formattées.
	 * 
	 * @param field Champ
	 * @return Valeur formattée (typée)
	 */
	private Object doGetTypedValue(final DtField dtField) {
		if (isModified(dtField)) {
			//Si le tableaux des valeurs formattées n'a pas été créeà la valeur est null.
		...
		for (final Map.Entry<String, Serializable> entry : modifiedTypedValues.entrySet()) {
			if (FORMAT_ERROR_VALUE.equals(entry.getValue())) {
				...
				try {
					...
					throw new VRuntimeException("Erreur de formatage non reproduite ('" + inputBuffer.get(constFieldName) + "'=>" + String.valueOf(typedValue) + "), l'UiObject doit être désynchronisé. Recharger votre page. " + this.toString());

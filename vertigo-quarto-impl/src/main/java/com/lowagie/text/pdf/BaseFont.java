...
public abstract class BaseFont {
	...
	protected void createEncoding() {
		if (encoding.startsWith("#")) {
			...
			final StringTokenizer tok = new StringTokenizer(encoding.substring(1), " ,\t\n\r\f");
	...
	public int getDescent(final String text) {
		...
		for (int k = 0; k < chars.length; ++k) {
			final int bbox[] = getCharBBox(chars[k]);
	...
	private static void recourseFonts(final PdfDictionary page, final IntHashtable hits, final ArrayList fonts, int level) {
		...
		if (font != null) {
			for (final Iterator it = font.getKeys().iterator(); it.hasNext();) {
				final PdfObject ft = font.get((PdfName) it.next());
		...
		if (xobj != null) {
			for (final Iterator it = xobj.getKeys().iterator(); it.hasNext();) {

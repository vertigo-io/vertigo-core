...
/**
 * This container contains params initialized with Propetries.
...
public final class PropertiesContainer implements Container {
	...
	public PropertiesContainer(final Properties params) {
		...
		keys = Collections.unmodifiableSet(new HashSet<>(unusedKeys));
	}
	 ...
	 */
	private static Object getParam(final Properties params, final String paramName, final Class<?> paramType) {
		Assertion.checkNotNull(params);
		Assertion.checkNotNull(paramName);
		//---------------------------------------------------------------------
		final String value = params.getProperty(paramName);
